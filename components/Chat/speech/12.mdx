...
Finally, let's display the detected word in the component's rendered output.

With these changes, the _`VoiceListener`_ component will display the detected word from the list whenever the user speaks it.
...
const VoiceListener = () => {
  ...
  return (
    <div>
      <h1>Voice Listener</h1>
      {detectedWord && <p>Detected word: {detectedWord}</p>}
    </div>
  )
}
